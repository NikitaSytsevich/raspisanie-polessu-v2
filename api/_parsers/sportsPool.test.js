...
test('sportsPool: блок «Обучение плаванию» после сетки не протекает в последний день', () => {
  // Регресс: воскресенье — последний день, его срез тянулся до конца текста и
  // заглатывал занятия «Вторник/Четверг 19:00, 20:00» → лишние сеансы.
  const out = parse(loadFixture('sports_pool_trailing_lessons.html'), { todayIso: TODAY });
  assert.equal(out.ok, true);

  const sun = out.sessions.filter(s => s.date === '2026-06-07').map(s => `${s.start}-${s.end}`);
  assert.deepEqual(
    sun.sort(),
    ['09:15-10:00', '10:30-11:15', '18:00-18:45', '20:30-21:15'],
    `воскресенье должно содержать ровно 4 сеанса сетки, got: ${sun}`);
  // Конкретно — времена из блока занятий не должны попасть в воскресенье.
  assert.ok(!sun.includes('19:00-19:45'), '19:00 из «Обучения» протёк в воскресенье');
  assert.ok(!sun.includes('20:00-20:45'), '20:00 из «Обучения» протёк в воскресенье');
  // Время в самой шапке «Обучение плаванию … 07.00» (ДО слова-дня) тоже
  // отрезается — секция игнорируется целиком, а не только по границе дня.
  assert.ok(!sun.includes('07:00-07:45'), '07:00 из шапки «Обучения» протёк в воскресенье');

  // Суббота (НЕ последний день) тоже не должна тащить занятия.
  const sat = out.sessions.filter(s => s.date === '2026-06-06').map(s => `${s.start}-${s.end}`);
  assert.deepEqual(
    sat.sort(),
    ['09:15-10:00', '15:30-16:15', '18:00-18:45'],
    `суббота должна содержать ровно 3 сеанса сетки, got: ${sat}`);

  // Совсем не должно быть сессий вне двух перечисленных дней
  // (никаких «дней-слов» из блока занятий как отдельных дат).
  const dates = [...new Set(out.sessions.map(s => s.date))].sort();
