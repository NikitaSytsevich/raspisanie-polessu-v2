...
// Превращает «(свободно 3 дорожки, без 1 крайней)» → «Свободное плавание · 3 дорожки».
// Без скобок или без матча — просто «Свободное плавание».
function activityFromDescription(desc) {
  ...
  const m = normalizeText(desc).match(/свободно\s+(\d+)\s+(дорож[а-яё]+)/iu);
  if (!m) return base;
  return `${base} · ${m[1]} ${m[2]}`;
}
