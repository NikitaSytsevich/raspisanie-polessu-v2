...
- **Telegram-бот** — не только уведомления, но и команды:
  ...
  - `/api/telegram` — webhook бота: `/today`, `/tomorrow`, `/status`,
    ...
    `TELEGRAM_WEBHOOK_SECRET`. В группах бот игнорирует команды, адресованные
    другим ботам (`/cmd@ДругойБот`), и не отвечает на неизвестные команды;
    чаты, заблокировавшие бота, автоматически выписываются из рассылки.
    `/today` отвечает из минутного кэша снапшота, а не 4 живыми фетчами.
