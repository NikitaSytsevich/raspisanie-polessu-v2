...
- **Telegram-бот** — не только уведомления, но и команды:
  - `/api/notify` сверяет сайт и шлёт изменения (Vercel Cron раз в сутки +
    ...
    название каждого объекта в сообщении — ссылка на его страницу расписания
    на polessu.by, длинные сообщения
    режутся по лимиту 4096 c retry на 429. Гребная база (weeklyPattern)
    сверяется по недельному паттерну — скользящие даты «Пн-Пт» не дают
    фантомных изменений, а реальные (время, выходной, закрытие) доходят.
    Переходы «работает ↔ закрыт» приходят отдельной строкой
    («⛔ закрыт: …» / «✅ снова работает») вместо флуда ➖/➕ по всем сеансам;
    объекты с не-ok качеством данных с любой из сторон в diff не участвуют.
    Уведомления фильтруются по объектам, выбранным в `/settings` каждым чатом;
    ночью (22:00–08:00 Минск) приходят без звука (`TELEGRAM_QUIET=off` отключает).
  - `/api/telegram` — webhook бота: `/today`, `/tomorrow`, `/now` (что идёт
    сейчас + свободные дорожки большого бассейна), `/week`, `/status`,
    `/settings`, `/subscribe`, `/unsubscribe`. Под расписанием — инлайн-кнопки
    навигации по дням («‹ день · 🔄 · день ›», «Неделя», «Сегодня»), правящие
    то же сообщение (`callback_query` + `editMessageText`). `/settings` — меню:
    какие объекты слать в уведомлениях и тумблер дайджеста (хранятся в Blob).
    Регистрация: `setWebhook` c `secret_token` = `TELEGRAM_WEBHOOK_SECRET` и
    `allowed_updates` с `callback_query` — после деплоя дёрнуть `?setup=` раз.
    В группах бот игнорирует команды, адресованные другим ботам
    (`/cmd@ДругойБот`), и не отвечает на неизвестные; заблокировавшие бота чаты
    выписываются из рассылки. `/today` и компания отвечают из кэша снапшота
    (память инстанса + общий Blob-снапшот ~2 мин), а не 4 живыми фетчами.
  ...
  - Утренний дайджест: первый прогон notify в окне 07:50–08:35 Минска шлёт
    расписание на день (глобально выключается `TELEGRAM_DIGEST=off`, по чату —
    тумблером в `/settings`; уважает выбранные объекты).
  - env: `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, `CRON_SECRET`,
    ...
    `APP_URL` (канонический адрес приложения — хост вебхука). Токены —
    только в переменных окружения, не в коде.
...
- **Парсеры** — общий «табличный экстрактор» (`_common.js`) + общий inline-движок (`_inline.js`: «Вторник 26.05.2026 / HH.MM – HH.MM …» с конкретными датами — так публикуют расписание оба бассейна и ледовая арена после ремонтов) + детектор закрытия объекта + отдельный inline-парсер гребной базы (расписание в `<h1>` через `<br>`). Все сессии проходят валидацию (`start < end`, окно дат `[today−7, today+45]`), inline побеждает таблицу только при «богатом» результате (≥2 дней или ≥3 слотов). Парсер возвращает `{ ok: true, sessions, closureRanges?, weeklyPattern? }` либо `{ ok: false, reason: 'closed', notice, range }`. `weeklyPattern: true` ставят парсеры, синтезирующие даты из дней недели (гребная база, табличный фолбэк): их окно дат сдвигается каждый день само, поэтому diff сравнивает такие объекты по недельному паттерну (день недели + время), а не по конкретным датам — иначе каждый день были бы фантомные «➖ прошлый понедельник / ➕ следующий понедельник».
